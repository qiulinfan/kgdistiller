...
`agent` CLI commands and consume their JSON output. In qlblog the command is
...
```sh
python3 knowledge/kgd.py agent resolve "Concept A" "Concept B"
```
...
```sh
python3 knowledge/kgd.py agent align CANDIDATE \
  --output knowledge/build/reviews/NAME.alignment.json
python3 knowledge/kgd.py agent compare CANDIDATE
```
